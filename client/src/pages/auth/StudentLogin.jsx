...
export default function StudentLogin() {
  ...
  const onSubmit = async ({ identifier, password }) => {
    try {
      const data = await login('student', identifier, password);
      if (data.user.firstLogin && !data.user.admissionFormFilled) {
      ...
      >
        ...
        <div style={styles.card}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={styles.inputWrap}>
              ...
              <span style={styles.forgotLink}>
                <Link to="/forgot-password" style={{ color: '#F5A623', fontSize: 11, textDecoration: 'none' }}>
                  Forgot Password?
                </Link>
              </span>
            ...
            >
              {isSubmitting ? (
                <span style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
                  <div style={styles.spinner} /> Logging in...
                </span>
              ) : 'Login'}
            </motion.button>
          </form>
          ...
          <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.55)', fontSize: 13, marginTop: 16 }}>
            ...
            <Link to="/signup" style={{ color: '#F5A623', fontWeight: 600, textDecoration: 'none' }}>Sign Up</Link>
          </p>
          ...
          <div style={styles.socialRow}>
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} style={styles.socialBtn} id="google-login">
              <FaGoogle size={18} />
            </motion.button>
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} style={styles.socialBtn} id="facebook-login">
              <FaFacebookF size={18} />
            </motion.button>
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} style={styles.socialBtn} id="apple-login">
              <FaApple size={18} />
            </motion.button>
          </div>
...
const styles = {
  ...
  bgRadial3: {
    ...
    background: 'radial-gradient(ellipse at 50% 80%, rgba(245,120,10,0.07) 0%, transparent 60%)',
    pointerEvents: 'none',
  },
  corner: {
    position: 'absolute', width: 80, height: 80,
    border: '2px solid rgba(245,166,35,0.4)', pointerEvents: 'none',
  },
  ...
  inputArrow: {
    ...
    color: 'rgba(245,166,35,0.6)', fontSize: 20, pointerEvents: 'none',
  },
  eyeBtn: {
  ...
  loginBtn: {
    width: '100%',
    background: 'linear-gradient(135deg, #d4881a 0%, #F5A623 50%, #c97a10 100%)',
    color: '#1a0e00',
    fontWeight: 700,
    fontSize: 16,
    letterSpacing: 1,
