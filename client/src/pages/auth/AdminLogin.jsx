...
const schema = z.object({
  identifier: z.string().min(1, 'Email required'),
...
export default function AdminLogin() {
  ...
  const onSubmit = async ({ identifier, password }) => {
    try {
      const data = await login('admin', identifier, password);
    ...
    <div style={styles.page}>
      ...
      >
        ...
        <div style={styles.card}>
          {/* Admin badge */}
          <div style={styles.roleBadge}>🛡️ Admin Portal – Secured Access</div>
          <h1 style={styles.cardTitle}>Login to Your Account</h1>

          <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={styles.inputWrap}>
              <FiMail style={styles.inputIcon} size={16} />
              <input {...register('identifier')} placeholder="Email or Username" id="admin-identifier" style={styles.input} />
            ...
            <div style={styles.inputWrap}>
              ...
              <input {...register('password')} type={showPw ? 'text' : 'password'} placeholder="Password" id="admin-password" style={{ ...styles.input, paddingRight: 80 }} />
            ...
            <motion.button type="submit" disabled={isSubmitting} id="admin-login-btn" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} style={styles.loginBtn}>
          ...
          <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.45)', fontSize: 12, marginTop: 14 }}>
          ...
          <div style={styles.socialRow}>
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} style={styles.socialBtn} id="admin-google-login"><FaGoogle size={18} /></motion.button>
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} style={styles.socialBtn} id="admin-facebook-login"><FaFacebookF size={18} /></motion.button>
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} style={styles.socialBtn} id="admin-apple-login"><FaApple size={18} /></motion.button>
          </div>

          <div style={{ marginTop: 18, display: 'flex', gap: 10, justifyContent: 'center' }}>
            <Link to="/login/student" style={styles.roleLink}>Student Login</Link>
            <span style={{ color: 'rgba(255,255,255,0.2)' }}>|</span>
            <Link to="/login/teacher" style={styles.roleLink}>Teacher Login</Link>
          </div>
        </div>
      </motion.div>
    </div>
