...
const emitNotificationAndSocket = async ({ req, recipients, title, message, type, data }) => {
	...
	// Mongo + Socket.io + Expo push (same path as announcements/leave)
...
const createDoubt = async (req, res) => {
	try {
		...
		if (userRole === 'student') {
			// Student-initiated doubt (existing logic — unchanged)
			...
			assignedTeachers = [req.user._id]; // Creating teacher is assigned
		...
		await createAuditLog({
			...
			metadata: { assignedTeachers: assignedTeachers.length, initiatedBy: userRole },
		});
