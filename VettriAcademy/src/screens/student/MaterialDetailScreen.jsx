...
export default function MaterialDetailScreen({ route, navigation }) {
  ...
  // ─── Download Handler ───────────────────────────────────────────────────────
  // Uses Linking.openURL to trigger download via the browser.
  // The backend returns a Cloudinary URL with fl_attachment flag which sets
  // the correct Content-Disposition header (filename + extension).
  const handleDownload = async () => {
    ...
    try {
      ...
      // Check if we already have a cached download URL for this material
      let downloadUrlObj = null;
      if (currentDownloadUrl?.url && currentDownloadUrl?.id === material._id) {
        downloadUrlObj = currentDownloadUrl;
      } else {
        const result = await dispatch(fetchDownloadUrl(material._id));
        if (fetchDownloadUrl.fulfilled.match(result)) {
          downloadUrlObj = result.payload;
        } else {
          Toast.show({ type: 'error', text1: 'Download Failed', text2: result.payload || 'Cannot fetch material' });
          return;
        }
      }

      if (downloadUrlObj) {
        let fullUrl = buildFullUrl(downloadUrlObj.url);
        
        // If it's a backend endpoint, append token for auth
    ...
    <View style={[styles.container, { backgroundColor: bgColor, padding: 16 }]}>
      ...
      <View style={styles.actionRow}>
        ...
        <TouchableOpacity
          ...
          disabled={downloadLoading || material?.isLocked || downloading}
        >
          {downloadLoading || downloading ? <ActivityIndicator color={Colors.white} /> : (
            <>
