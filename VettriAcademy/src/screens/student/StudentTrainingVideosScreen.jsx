...
import {
  getStudentTrainingVideosAPI,
  markStudentVideoCompleteAPI,
  getStudentIncompleteMandatoryCountAPI,
} from '../../services/api';
...
export default function StudentTrainingVideosScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    try {
      const [vRes, cRes] = await Promise.all([getStudentTrainingVideosAPI(), getStudentIncompleteMandatoryCountAPI()]);
  ...
  const markDone = async (v) => {
    try {
      setMarking(v._id);
      await markStudentVideoCompleteAPI(v._id, v.duration);
    ...
    <View style={[styles.container, { backgroundColor: bg }]}>
      <LinearGradient colors={Colors.gradient.primary} style={styles.header}>
        <View style={styles.hRow}>
          <View style={{ flex: 1 }}><Text style={styles.hTitle}>📚 Training Hub</Text><Text style={styles.hSub}>Learn at your own pace</Text></View>
        </View>
      </LinearGradient>
