    ...
    <>
      ...
      <View style={st.heroWrap}>
        ...
        <View style={[st.heroBar, { paddingTop: insets.top + 8 }]}>
          ...
          <View style={st.heroBarRight}>
            <TouchableOpacity style={st.heroIconBtn} onPress={() => dispatch(toggleAI())} activeOpacity={0.85}>
              <Ionicons name="sparkles" size={20} color={T.white} />
            </TouchableOpacity>
            <TouchableOpacity style={st.heroIconBtn} onPress={() => navigation.navigate('Notifications')} activeOpacity={0.85}>
              <Ionicons name="notifications-outline" size={22} color={T.white} />
              {unreadCount > 0 && (
                <View style={st.heroBadge}>
                  <Text style={st.heroBadgeText}>{unreadCount > 9 ? '9+' : unreadCount}</Text>
                </View>
              )}
            </TouchableOpacity>
          </View>
      ...
      <View style={st.statsGrid}>
        {[
          { emoji: '📚', label: 'Total Materials', value: stats.total, tint: T.pink },
          { emoji: '📄', label: 'PDFs', value: stats.pdf, tint: T.orange },
          { emoji: '🎥', label: 'Videos', value: stats.video, tint: T.teal },
          { emoji: '🖼', label: 'Images', value: stats.image, tint: T.purple },
        ].map((s) => (
          <View key={s.label} style={st.statCard}>
            <Text style={st.statEmoji}>{s.emoji}</Text>
            <Text style={[st.statValue, { color: s.tint }]}>{s.value}</Text>
...
const st = StyleSheet.create({
  ...
  materialCard: {
    ...
    height: 110, backgroundColor: T.white, borderRadius: 24,
  ...
  materialTitle: { fontSize: 15, fontWeight: '900', color: T.title },
  materialSubject: { fontSize: 12, fontWeight: '600', color: T.teal, marginTop: 3 },
