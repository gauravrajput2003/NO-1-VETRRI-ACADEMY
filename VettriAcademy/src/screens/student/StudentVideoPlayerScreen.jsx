...
import { View, Text, StyleSheet, TouchableOpacity as RNTouchableOpacity, ActivityIndicator, StatusBar, Platform } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { Colors } from '../../utils/colors';
import { updateStudentWatchProgressAPI, markStudentVideoCompleteAPI } from '../../services/api';
...
export default function StudentVideoPlayerScreen({ route, navigation }) {
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      ...
      setElapsed(secs);
      if (secs > 0 && secs % 30 === 0) {
        updateStudentWatchProgressAPI(video._id, secs, secs).catch(() => {});
      }
    }, 1000);
    return () => {
      clearInterval(intervalRef.current);
      const final = Math.floor((Date.now() - startRef.current) / 1000);
      updateStudentWatchProgressAPI(video._id, final, final).catch(() => {});
    };
  }, [video._id]);

  const handleComplete = async () => {
    try {
      await markStudentVideoCompleteAPI(video._id, elapsed || video.duration);
      navigation.goBack();
