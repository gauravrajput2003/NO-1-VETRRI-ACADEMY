...
export default function ClassSchedulerScreen({ navigation }) {
  ...
  const resetForm = () => {
    setForm({
      ...
      scheduledDate: '', scheduledTime: '', durationMinutes: '60', notes: '',
    });
    setSelectedStudents([]);
...
function ScheduleForm({
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F8F7FC' }}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        >
          ...
          <SectionBlock section={S[3]}>
            <FieldCard
              icon="document-text-outline" iconColor={S[3].accent}
      ...
      {/* ── BOTTOM ACTION BAR ── */}
      <View style={[formStyles.actionBar, { paddingBottom: Math.max(insets.bottom, 16) }]}>
